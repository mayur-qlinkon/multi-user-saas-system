...
class StorefrontSection extends Model
{
    ...
    {
        if ($this->view_count === 0) return 0.0;
    ...
    {
        if ($this->view_all_url) return $this->view_all_url;
     ...
     */
    public function resolveProducts(int $limit = null): \Illuminate\Database\Eloquent\Collection
    {
     ...
     */
    private function resolveCategory(int $limit): \Illuminate\Database\Eloquent\Collection
    {
        if (!$this->category_id) return new \Illuminate\Database\Eloquent\Collection();
    ...
    private function resolveBanners(): \Illuminate\Database\Eloquent\Collection
    {
        ...
        return \App\Models\Banner::where('company_id', $this->company_id)
    ...
    private function resolveManual(int $limit): \Illuminate\Database\Eloquent\Collection
    {
        return Product::withoutGlobalScope('tenant')
            ->whereHas('sectionPivots', fn($q) =>
                $q->where('storefront_section_id', $this->id)
            )
            ->where('products.company_id', $this->company_id)
            ->where('products.is_active', true)
            ->where('products.show_in_storefront', true)
    ...
    {
        if (empty($sectionIds)) return true;
     ...
     */
    public static function getLiveForCompany(int $companyId): \Illuminate\Database\Eloquent\Collection
    {
    ...
    public function trackView(): void
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::warning('[StorefrontSection] View track failed', [
                'section_id' => $this->id,
                'error'      => $e->getMessage(),
            ]);
        }
    ...
    public function trackClick(): void
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::warning('[StorefrontSection] Click track failed', [
                'section_id' => $this->id,
                'error'      => $e->getMessage(),
            ]);
        }
