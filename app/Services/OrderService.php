...
class OrderService
{
    ...
    public function placeOrder(array $data, int $companyId): Order
    {
        return DB::transaction(function () use ($data, $companyId) {
            ...
            app(\App\Services\CrmLeadService::class)
   ...
   // ════════════════════════════════════════════════════
    ...
    private function resolveCartItems(array $cartItems, int $companyId): array
    {
        ...
        foreach ($cartItems as $index => $item) {
            ...
            $sku = ProductSku::whereHas('product', fn($q) =>
    ...
    {
        try {
            return \App\Models\Company::find($companyId)?->state?->name
                ?? \App\Models\Company::find($companyId)?->city
                ?? null;
        } catch (Throwable $e) {
