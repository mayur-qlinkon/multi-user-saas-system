...
class CrmLeadService
{
    ...
    {        
        ...
        $query = CrmLead::query()
            ...
            ->withCount(['tasks as pending_tasks_count' => fn($q) =>
                $q->whereIn('status', ['pending', 'in_progress'])
            ]);

        if (!empty($filters['q']))           $query->search($filters['q']);
        if (!empty($filters['pipeline_id'])) $query->byPipeline($filters['pipeline_id']);
        if (!empty($filters['stage_id']))    $query->byStage($filters['stage_id']);
        if (!empty($filters['priority']))    $query->byPriority($filters['priority']);
        if (!empty($filters['source_id']))   $query->where('crm_lead_source_id', $filters['source_id']);
        if (!empty($filters['mine']))        $query->assigned(Auth::id());
        if (!empty($filters['assigned_to'])) $query->assigned($filters['assigned_to']);
        if (!empty($filters['unassigned']))  $query->unassigned();
        ...
        if (!empty($filters['tag_id'])) {
            $query->whereHas('tags', fn($q) =>
                $q->where('crm_tags.id', $filters['tag_id'])
            );
        }
    ...
    public function updateLead(CrmLead $lead, array $data): CrmLead
    {
        return DB::transaction(function () use ($lead, $data) {
            ...
            if ($tags !== null)       $this->syncTags($lead, $tags);
            if ($assignedTo !== null) $lead->assignTo($assignedTo);
    ...
    public function createTask(CrmLead $lead, array $data): CrmTask
    {
        ...
        if (empty($data['remind_at']) && !empty($data['due_at'])) {
            $task->updateQuietly([
                'remind_at' => \Carbon\Carbon::parse($data['due_at'])->subHour(),
            ]);
        }
    ...
    private function applyInitialScore(CrmLead $lead): void
    {
        ...
        if ($lead->lead_value)   $score += 10;
        if ($lead->order_id)     $score += CrmLead::SCORE_ORDER_PLACED;
