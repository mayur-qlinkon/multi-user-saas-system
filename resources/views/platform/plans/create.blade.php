...
@section('title', 'Create Plan - Qlinkon')
...
@section('content')
    <div class="w-full pb-10" x-data="planFormManager(@js($modules->pluck('id')))">
        ...
        <form action="{{ route('platform.plans.store') }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            @csrf
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="name" value="{{ old('name') }}" required placeholder="e.g. Premium Plan" class="form-input">
                        </div>
                        <div class="md:col-span-2">
                            <label class="form-label">Short Description</label>
                            <input type="text" name="description" value="{{ old('description') }}" placeholder="e.g. Best for growing businesses" class="form-input">
                        </div>
                        
                        <div>
                            <label class="form-label">Price (₹) <span class="text-red-500">*</span></label>
                            <input type="number" step="0.01" name="price" value="{{ old('price', 0) }}" required class="form-input">
                        </div>
                        <div>
                            ...
                            <select name="billing_cycle" required class="form-input bg-white">
                                ...
                                <option value="lifetime" {{ old('billing_cycle') == 'lifetime' ? 'selected' : '' }}>Lifetime (One-time)</option>
                            </select>
                        </div>
                        <div>
                            <label class="form-label">Free Trial Days</label>
                            <input type="number" name="trial_days" value="{{ old('trial_days', 0) }}" min="0" class="form-input">
                        </div>
                        <div class="mt-2 md:mt-8">
                            <label class="flex items-center gap-2 cursor-pointer w-max">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-4 h-4 text-brand-600 rounded border-gray-300 focus:ring-brand-500">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="number" name="user_limit" value="{{ old('user_limit', 1) }}" required min="1" class="form-input">
                        </div>
                        <div>
                            <label class="form-label">Store Limit <span class="text-red-500">*</span></label>
                            <input type="number" name="store_limit" value="{{ old('store_limit', 1) }}" required min="1" class="form-input">
                        </div>
                        <div>
                            <label class="form-label">Product Limit <span class="text-red-500">*</span></label>
                            <input type="number" name="product_limit" value="{{ old('product_limit', 50) }}" required min="1" class="form-input">
                        </div>
                        <div>
                            <label class="form-label">Employee Limit <span class="text-red-500">*</span></label>
                            <input type="number" name="employee_limit" value="{{ old('employee_limit', 50) }}" required min="1" class="form-input">
                        </div>
                        <div>
                            <label class="form-label">Daily OCR Scans <span class="text-red-500">*</span></label>
                            <input type="number" name="ocr_scan_limit" value="{{ old('ocr_scan_limit', 50) }}" required min="0" class="form-input">
                        </div>
                    </div>
                </div>
    ...
    <script>
        function planFormManager(allModuleIds) {
            return {
                ...
                selectedModules: @json(old('modules', [])),
