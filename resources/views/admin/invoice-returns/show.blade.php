    ...
    <div class="pb-10">
        ...
        <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-2xl font-bold text-[#212538] tracking-tight">Credit Note Details</h1>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2">
                @if ($invoiceReturn->status === 'draft')
                    ...
                    <form action="{{ route('admin.invoice-returns.confirm', $invoiceReturn->id) }}" method="POST"
                        ...
                        @csrf
                        <button type="submit"
                            class="bg-[#108c2a] hover:bg-[#0c6b1f] text-white px-6 py-2 rounded-lg text-sm font-bold transition-all shadow-md flex items-center gap-2">
                            <i data-lucide="check-circle" class="w-4 h-4"></i> Confirm & Update Stock
                        </button>
                    </form>
                @endif
        ...
        <div id="print-area"
            ...
            <div class="p-8 md:p-12">
                ...
                <div class="p-8 pb-4 flex justify-between items-start">
                    <div>
                        <div
                            class="inline-block bg-red-600 text-white px-3 py-1 text-[10px] font-black uppercase tracking-widest mb-3">
                            Credit Note
                        </div>
                    ...
                    <div class="space-y-1">
                        <div class="grid grid-cols-2 text-[13px]">
                            ...
                            <span class="text-right font-bold text-blue-600 underline">
                                <a
                                    href="{{ route('admin.invoices.show', $invoiceReturn->invoice_id) }}">{{ $invoiceReturn->invoice->invoice_number }}</a>
                            </span>
                        </div>
                        <div class="grid grid-cols-2 text-[13px]">
                        ...
                        <div class="grid grid-cols-2 text-[13px]">
                            ...
                            <span
                                class="text-right font-black text-red-600 uppercase">{{ $invoiceReturn->return_type }}</span>
                        </div>
                        <div class="grid grid-cols-2 text-[13px]">
                            <span class="font-bold text-gray-500">Return Reason:</span>
                            <span
                                class="text-right font-bold text-gray-900 uppercase">{{ str_replace('_', ' ', $invoiceReturn->return_reason) }}</span>
                        </div>
                        ...
                        <div class="grid grid-cols-2 text-[13px]">
                            ...
                            <span
                                class="text-right font-black {{ $invoiceReturn->stock_updated ? 'text-green-600' : 'text-amber-500' }}">
                ...
                <div class="px-8 pb-12 flex justify-between items-start gap-10">

                    {{-- Left side: Notes (Takes up the rest of the space) --}}
                    <div class="flex-1">
                    ...
                    <div style="width: 300px;" class="flex flex-col items-end">
                        <table class="w-full text-[13px] border-collapse">
                            <tr>
                                <td class="py-1.5 text-gray-600 font-semibold">Taxable Subtotal</td>
                                <td class="py-1.5 text-right font-bold text-gray-900 whitespace-nowrap">
                                    ₹{{ $formatAmt($invoiceReturn->subtotal) }}</td>
                            </tr>
                            <tr>
                                <td class="py-1.5 text-gray-600 font-semibold">GST Reversal</td>
                                <td class="py-1.5 text-right font-bold text-gray-900 whitespace-nowrap">
                                    ₹{{ $formatAmt($invoiceReturn->tax_amount) }}</td>
                            </tr>
                            {{-- Thick line for the total --}}
                            <tr class="border-t-2 border-gray-900">
                                <td class="py-3 text-[14px] font-black text-gray-900 uppercase">Total Refund</td>
                                <td class="py-3 text-right text-[16px] font-black text-red-600 whitespace-nowrap">
                                    ₹{{ $formatAmt($invoiceReturn->grand_total) }}
                                </td>
                            </tr>
                        </table>

                        {{-- Signature section directly below the narrow table --}}
                        <div class="mt-20 text-right w-full">
                            <div class="inline-block min-w-[200px] text-center">
                                <div
                                    class="border-t border-gray-400 pt-1 text-[11px] font-bold text-gray-800 uppercase tracking-wider">
                                    Authorized Signatory
                                </div>
                                <div
                                    class="mt-1 text-[9px] text-gray-400 font-bold uppercase tracking-widest leading-none">
                                    {{ config('app.name') }} ERP
                                </div>
                            </div>
                        </div>
