...
@section('title', 'Edit Warehouse - Qlinkon BIZNESS')
...
@section('content')
    <div class="w-full mx-auto space-y-6 pb-10" x-data="warehouseEditForm()">
        ...
        {{-- Form --}}
        <form action="{{ route('admin.warehouses.update', $warehouse->id) }}" method="POST" class="space-y-6" @submit="isSubmitting = true">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $warehouse->name) }}" required placeholder="e.g. Main Godown"
                    ...
                    <div>
                        ...
                        <select name="store_id" required
                            ...
                            @foreach ($stores as $store)
                                <option value="{{ $store->id }}" {{ old('store_id', $warehouse->store_id) == $store->id ? 'selected' : '' }}>
                                    {{ $store->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="text" name="contact_person" value="{{ old('contact_person', $warehouse->contact_person) }}" placeholder="John Doe"
                            class="w-full border border-gray-300 rounded-md px-3.5 py-2.5 text-sm focus:border-[#108c2a] focus:ring-1 focus:ring-[#108c2a] outline-none transition-all">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', $warehouse->email) }}" placeholder="warehouse@example.com"
                            class="w-full border border-gray-300 rounded-md px-3.5 py-2.5 text-sm focus:border-[#108c2a] focus:ring-1 focus:ring-[#108c2a] outline-none transition-all">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" name="city" value="{{ old('city', $warehouse->city) }}" placeholder="e.g. Ahmedabad"
                            class="w-full border border-gray-300 rounded-md px-3.5 py-2.5 text-sm focus:border-[#108c2a] focus:ring-1 focus:ring-[#108c2a] outline-none transition-all">
                    </div>
                    <div>
                        ...
                        <select name="state_id"
                            ...
                            @foreach ($states as $state)
                                <option value="{{ $state->id }}" {{ old('state_id', $warehouse->state_id) == $state->id ? 'selected' : '' }}>
                                    {{ $state->name }}
                                </option>
                            @endforeach
                        </select>
