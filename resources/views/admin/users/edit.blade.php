...
@section('title', 'Edit Staff Member - Qlinkon BIZNESS')

@section('header-title')
    <h1 class="text-sm font-bold text-gray-500 uppercase tracking-widest">LIST / USERS / EDIT</h1>
@endsection
...
@section('content')
    <div class="pb-10 w-full">
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">Update account details and store access for <strong>{{ $user->name }}</strong>.</p>
                </div>
            </div>
        </div>
        ...
        {{-- Main Form Container --}}
        <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="w-full bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 md:p-8 border-b border-gray-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $user->name) }}" required placeholder="e.g. Jane Doe"
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email', $user->email) }}" required placeholder="jane@example.com"
                            class="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none transition-all shadow-sm">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" required class="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none transition-all bg-white shadow-sm appearance-none">
                            ...
                            <option value="inactive" @selected(old('status', $user->status) === 'inactive')>Inactive (Suspended)</option>
                        </select>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    @forelse($stores as $store)
                        <label class="block relative cursor-pointer group">
                            {{-- Check old input first (on validation failure), otherwise use database relationships --}}
                            <input type="checkbox" name="store_ids[]" value="{{ $store->id }}" class="peer sr-only" 
                                @checked(is_array(old('store_ids')) ? in_array($store->id, old('store_ids')) : in_array($store->id, $userStoreIds))>
