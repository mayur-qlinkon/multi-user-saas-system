    ...
    <div class="pb-10">
        ...
        <div id="print-area" class="bg-white rounded shadow-sm border border-gray-200 overflow-hidden text-[#475569]">
            ...
            <div class="text-center py-6 border-b border-gray-100">
                ...
                <p class="text-[12px] text-gray-500 mt-1">No: {{ $purchaseReturn->return_number }}</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 print-grid-3 gap-6 p-6 print:p-2">
                ...
                <div>
                    ...
                    <div class="px-2 space-y-2 text-[13px]">
                        <div class="flex items-start gap-2">
                            ...
                            <span class="text-gray-800 font-bold">{{ $purchaseReturn->supplier->name ?? 'N/A' }}</span>
                        </div>
                ...
                <div>
                    <div class="bg-[#f1f5f9] px-3 py-2 text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-3">
                        Company Info</div>
                    <div class="px-2 space-y-2 text-[13px]">
                        @php $companyInfo = $purchaseReturn->store ?? auth()->user()->company; @endphp
                        <div class="flex items-start gap-2">
                            <i data-lucide="building" class="w-4 h-4 shrink-0 mt-0.5"></i>
                            <span class="text-gray-800 font-bold">{{ $companyInfo->name ?? 'N/A' }}</span>
                        </div>
                ...
                <div>
                    ...
                    <div class="px-2 space-y-2 text-[13px]">
                        <div class="grid grid-cols-[100px_1fr] items-center">
                            ...
                            <a href="{{ route('admin.purchases.show', $purchaseReturn->purchase_id) }}"
                                class="text-[#108c2a] font-bold no-print hover:underline">{{ $purchaseReturn->purchase->purchase_number }}</a>
                            <span
                                class="text-gray-800 font-bold hidden print:inline">{{ $purchaseReturn->purchase->purchase_number }}</span>
                        </div>
                        <div class="grid grid-cols-[100px_1fr] items-center">
                            <span class="text-gray-500">Date:</span>
                            <span class="text-gray-800">{{ $purchaseReturn->return_date->format('d M Y') }}</span>
                        </div>
                        <div class="grid grid-cols-[100px_1fr] items-center">
                            <span class="text-gray-500">Warehouse:</span>
                            <span class="text-gray-800">{{ $purchaseReturn->warehouse->name ?? 'N/A' }}</span>
                        </div>
                        <div class="grid grid-cols-[100px_1fr] items-center">
                            ...
                            @foreach ($purchaseReturn->items as $item)
                                <tr>
                                    <td class="py-3 px-2 text-gray-800">
                                        <div class="font-bold">{{ $item->product->name ?? 'Unknown' }}</div>
                                        <div class="text-[11px] text-gray-500 font-mono">
                                            {{ $item->productSku->sku ?? 'N/A' }}</div>
                                    </td>
                                    ...
                                    <td class="py-3 px-2 text-center font-bold text-red-500 printState">
                                        -{{ $formatAmt($item->quantity) }}</td>
                                    <td class="py-3 px-2 text-center text-gray-600">₹ {{ $formatAmt($item->tax_amount) }}
                                    </td>
                                    <td class="py-3 px-2 text-right font-bold text-gray-800">₹
                                        {{ $formatAmt($item->total_price) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="py-2 text-right border-b border-gray-100 font-medium text-gray-800">₹
                                    {{ $formatAmt($purchaseReturn->taxable_amount) }}</td>
                            </tr>
                            <tr>
                                <td class="py-2 border-b border-gray-100">Total Tax Reversal</td>
                                <td class="py-2 text-right border-b border-gray-100 font-medium text-gray-800">₹
                                    {{ $formatAmt($purchaseReturn->tax_amount) }}</td>
                            </tr>
                            <tr>
                                ...
                                <td class="py-3 text-right text-[#108c2a] font-bold printState text-[15px]">₹
                                    {{ $formatAmt($purchaseReturn->total_amount) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            @if ($purchaseReturn->reason || $purchaseReturn->supplier_credit_note_number || $purchaseReturn->notes)
                <div class="px-6 pb-8 print:pb-2 print:pt-2 text-[13px] text-gray-500 page-break-avoid">
                    ...
                    @if ($purchaseReturn->supplier_credit_note_number)
                        <div class="mb-3 bg-blue-50/50 p-3 rounded border border-blue-100">
                            ...
                            <span
                                class="text-blue-900 font-mono">{{ $purchaseReturn->supplier_credit_note_number }}</span>
                        </div>
                    @endif

                    @if ($purchaseReturn->reason)
                        <p class="mb-2"><strong class="text-gray-700">Reason for Return:</strong>
                            {{ $purchaseReturn->reason }}</p>
                    @endif

                    @if ($purchaseReturn->notes)
                        <p><strong class="text-gray-700">Additional Notes:</strong> {{ $purchaseReturn->notes }}</p>
                    @endif
                </div>
            @endif
