...
@endsection
@section('content')
    <div class="pb-10" x-data="userManager(@js($users), @js($stores), @js($roles))">

        <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                {{-- <h1 class="text-2xl font-bold text-[#212538] tracking-tight">Staff Management</h1> --}}
       ...
       {{-- OWNER ONLY: Store Filter --}}
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div
                                        ...
                                        @if (auth()->id() !== $user->id && ($user->roles->first()->slug ?? '') !== 'owner')
                                            <button type="button" @click="openEdit({{ $user->id }})"
                                        ...
                                        {{-- Hide delete if it's the current user OR if the target user is an owner --}}
                                        @if (auth()->id() !== $user->id && ($user->roles->first()->slug ?? '') !== 'owner')
                                            <button type="button"
        ...
        <div x-cloak x-show="showFormModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]"
                ...
                <form :action="isEditing ? `/admin/users/${form.id}` : '{{ route('admin.users.store') }}'" method="POST"
