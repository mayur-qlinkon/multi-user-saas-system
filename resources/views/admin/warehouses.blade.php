                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse ($warehouses as $warehouse)
                            <tr class="hover:bg-gray-50/50 transition-colors"
                                x-show="matchesSearch('{{ strtolower($warehouse->name) }}', '{{ strtolower($warehouse->city) }}')">
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="font-bold text-[#475569] text-[13.5px]">{{ $warehouse->name }}</span>
                                        <span
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span
                                            class="font-bold text-gray-700 text-[13px]">{{ $warehouse->contact_person ?? 'Not Set' }}</span>
                                        <span
                                            class="text-[12px] text-gray-400 font-medium">{{ $warehouse->phone ?? '-' }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-1.5 text-gray-500 font-medium italic">
                                        ...
                                        {{ $warehouse->city ?? 'N/A' }}
                                        {{ $warehouse->state_id ? ', ' . $warehouse->state->name : '' }}
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <button @click="openEditModal({{ $warehouse->toJson() }})"
    ...
    <script>
        function warehouseCrud() {
            return {
                ...
                isModalOpen: false,
                modalMode: 'create',
                formAction: '{{ route('admin.warehouses.store') }}',
                formData: {
                    store_id: '',
                    name: '',
                    contact_person: '',
                ...
                openCreateModal() {
                    ...
                    this.formAction = '{{ route('admin.warehouses.store') }}';
                    this.formData = {
                        store_id: '',
                        name: '',
                        contact_person: '',
