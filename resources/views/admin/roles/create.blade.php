...
@section('title', 'Create Role - Qlinkon BIZNESS')
@section('header-title')
    <h1 class="text-sm font-bold text-gray-500 uppercase tracking-widest">Create / Roles</h1>
@endsection
@section('content')
    @php
    ...
    <div class="pb-10" x-data="roleMatrixForm(@js(old('permissions', [])), @js($jsMatrix))">
        ...
        <form action="{{ route('admin.roles.store') }}" method="POST" @submit="BizAlert.loading('Creating Role...')">
            @csrf
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors" 
                                    ...
                                    <td class="px-6 py-4 text-center bg-gray-50/30">
                                        @if (count($data['all_ids']) > 0)
                                            <input type="checkbox" @change="toggleRow('{{ $data['key'] }}')"
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if ($data['view'])
                                            <input type="checkbox" name="permissions[]" value="{{ $data['view']->id }}"
                                                x-model.number="selected"
                                                @change="checkDependencies('{{ $data['key'] }}', 'view', {{ $data['view']->id }})"
                                                class="w-4 h-4 rounded border-gray-300 text-[#108c2a] focus:ring-[#108c2a] transition-colors cursor-pointer">
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if ($data['create'])
                                            <input type="checkbox" name="permissions[]" value="{{ $data['create']->id }}"
                                                x-model.number="selected"
                                                @change="checkDependencies('{{ $data['key'] }}', 'action', {{ $data['create']->id }})"
                                                class="w-4 h-4 rounded border-gray-300 text-[#108c2a] focus:ring-[#108c2a] transition-colors cursor-pointer">
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if ($data['update'])
                                            <input type="checkbox" name="permissions[]" value="{{ $data['update']->id }}"
                                                x-model.number="selected"
                                                @change="checkDependencies('{{ $data['key'] }}', 'action', {{ $data['update']->id }})"
                                                class="w-4 h-4 rounded border-gray-300 text-[#108c2a] focus:ring-[#108c2a] transition-colors cursor-pointer">
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if ($data['delete'])
                                            <input type="checkbox" name="permissions[]" value="{{ $data['delete']->id }}"
                                                x-model.number="selected"
                                                @change="checkDependencies('{{ $data['key'] }}', 'action', {{ $data['delete']->id }})"
                                                class="w-4 h-4 rounded border-gray-300 text-[#108c2a] focus:ring-[#108c2a] transition-colors cursor-pointer">
                                        @endif
                                    </td>

                                    {{-- Other Checkboxes (Print, Export, Convert, etc.) --}}
                                                ...
                                                @foreach ($data['others'] as $other)
                                                    <label class="flex items-center gap-1.5 cursor-pointer">
                                                        <input type="checkbox" name="permissions[]"
                                                            value="{{ $other->id }}" x-model.number="selected"
                                                            @change="checkDependencies('{{ $data['key'] }}', 'action', {{ $other->id }})"
                                                            class="w-4 h-4 rounded border-gray-300 text-[#108c2a] focus:ring-[#108c2a] transition-colors cursor-pointer">
                                                        <span
                                                            class="text-[11px] font-medium text-gray-600 uppercase tracking-wider">{{ explode('.', $other->slug)[1] ?? $other->name }}</span>
                                                    </label>
                                                @endforeach
                                            </div>
                                        @endif
