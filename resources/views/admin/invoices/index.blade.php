...
@section('content')
    <div class="pb-10" x-data="invoiceIndex()">
        ...
        <div class="bg-white rounded-t-xl shadow-sm border border-gray-100 p-4 border-b-0">
            <form action="{{ route('admin.invoices.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
                ...
                <div x-data="{ filterOpen: false }" @click.away="filterOpen = false" class="relative">
                    <button type="button" @click="filterOpen = !filterOpen"
                        ...
                        @if (request('status') || request('payment_status') || request('source'))
                    ...
                    <div x-show="filterOpen" x-cloak x-transition
                        ...
                        <div class="mb-3">
                            ...
                            <select name="source"
                                ...
                                <option value="pos" {{ request('source') == 'pos' ? 'selected' : '' }}>POS Sale</option>
                                <option value="direct" {{ request('source') == 'direct' ? 'selected' : '' }}>Direct Invoice
                                </option>
                                <option value="online" {{ request('source') == 'online' ? 'selected' : '' }}>Online Order
                                </option>
                            </select>
                        </div>
                ...
                <table class="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @forelse ($invoices as $invoice)
                            {{-- 🌟 Calculate Balance Due for this row --}}
                            @php
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors group">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <span
                                        class="font-extrabold text-gray-800">₹{{ number_format($invoice->grand_total, 2) }}</span>
                                </td>
        ...
        <div x-show="isPaymentModalOpen" x-cloak
            ...
            <div class="bg-white w-full max-w-md mx-4 sm:mx-0 rounded-2xl shadow-2xl flex flex-col overflow-hidden"
                ...
                {{-- Dynamically bind the action URL to the specific invoice ID --}}
                <form :action="`/admin/invoices/${activePayment.invoice_id}/pay`" method="POST"
                    ...
                    <div class="p-6 space-y-5">
                        ...
                        <x-payment-method-select name="payment_method_id" label="Payment Mode"
                            x-model="activePayment.payment_method_id" required />
