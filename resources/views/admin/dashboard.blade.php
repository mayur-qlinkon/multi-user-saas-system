        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6 overflow-hidden">                                
            ...
            <div class="flex overflow-x-auto pb-1 hide-scrollbar gap-2 w-full lg:w-auto">
                ...
                @if(has_module('ocr_scanner') && has_permission('ocr_scanner.access'))
                    <a href="{{ route('admin.ocr-scanner.index') }}" class="whitespace-nowrap bg-white border border-gray-200 text-gray-700 px-4 py-2.5 rounded-xl text-sm font-bold shadow-sm hover:bg-gray-50 transition-colors flex items-center justify-center gap-2 shrink-0">
      ...
      {{-- 2. THE 8 FINANCIAL METRIC CARDS (Owner Only) --}}
        @if($is_owner)
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                ...
                {{-- Row 1: Monthly Stats --}}
                <a href="{{ route('admin.invoices.index') }}" class="stat-card bg-gradient-to-r from-cyan-400 to-blue-500 rounded-xl p-5 text-white relative overflow-hidden block">
                ...
                <a href="{{ route('admin.purchases.index') }}" class="stat-card bg-gradient-to-r from-purple-500 to-indigo-500 rounded-xl p-5 text-white relative overflow-hidden block">
                    <div class="flex justify-between items-start mb-2 relative z-10">
                ...
                <a href="{{ route('admin.invoice-returns.index') }}" class="stat-card bg-gradient-to-r from-orange-400 to-orange-500 rounded-xl p-5 text-white relative overflow-hidden block">
                    <div class="flex justify-between items-start mb-2 relative z-10">
                ...
                <a href="{{ route('admin.purchase-returns.index') }}" class="stat-card bg-gradient-to-r from-blue-400 to-cyan-500 rounded-xl p-5 text-white relative overflow-hidden block">
                    <div class="flex justify-between items-start mb-2 relative z-10">
                ...
                {{-- Row 2: Today Stats --}}
                <a href="{{ route('admin.invoices.index') }}" class="stat-card bg-gradient-to-r from-yellow-400 to-amber-500 rounded-xl p-5 text-white relative overflow-hidden block">
                    <div class="flex justify-between items-start mb-2 relative z-10">
                ...
                <a href="{{ route('admin.purchases.index') }}" class="stat-card bg-gradient-to-r from-red-500 to-rose-600 rounded-xl p-5 text-white relative overflow-hidden block">
                    <div class="flex justify-between items-start mb-2 relative z-10">
                ...
                <a href="{{ route('admin.expenses.index') }}" class="stat-card bg-gradient-to-r from-fuchsia-500 to-purple-600 rounded-xl p-5 text-white relative overflow-hidden block">
                    <div class="flex justify-between items-start mb-2 relative z-10">
       ...
       {{-- MAIN CONTENT SPLIT: Charts --}}
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden h-fit">
                    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <h2 class="text-sm font-black text-gray-800 uppercase tracking-wider">Top Selling Products ({{ now()->format('F') }})</h2>
                        </div>
                    </div>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @forelse ($charts['top_products'] as $product)
                                    <tr class="hover:bg-gray-50/50 transition-colors">
                                        <td class="px-6 py-4 font-semibold text-gray-600">{{ $product->product_name }}</td>
                                        ...
                                        <td class="px-6 py-4 text-right font-medium text-gray-700">₹ {{ $formatAmt($product->total_revenue) }}</td>
                                    </tr>
                                @empty
                                    <tr>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-8">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                ...
                <a href="{{ route('admin.invoices.index') }}" class="text-xs font-bold text-brand-600 hover:text-brand-700 transition-colors">View All &rarr;</a>
            </div>
            <div class="overflow-x-auto custom-scrollbar">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse ($tables['recent_sales'] as $sale)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right font-black text-gray-900">₹ {{ $formatAmt($sale['grand_total']) }}</td>
                                <td class="px-6 py-4 text-right font-semibold text-gray-600">₹ {{ $formatAmt($sale['paid']) }}</td>
                                <td class="px-6 py-4 text-right font-bold {{ $sale['due'] > 0 ? 'text-red-500' : 'text-gray-400' }}">
                                    ₹ {{ $formatAmt($sale['due']) }}
                                </td>
                                <td class="px-6 py-4 text-center">
                                    @php
                                    ...
                                    @endphp
                                    <span class="px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider {{ $payBadge }}">
                                        {{ $sale['payment_status'] }}
                                    </span>
                                </td>
                            </tr>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-red-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-red-100 bg-red-50/50 flex items-center justify-between">
                ...
                @if ($tables['low_stock_skus']->count() > 0)
                    <a href="{{ route('admin.inventory.reports.index') }}" class="text-xs font-bold text-red-600 hover:text-red-700 transition-colors">Inventory Report &rarr;</a>
                @endif
            </div>
            
            <div class="overflow-x-auto custom-scrollbar">
                @if ($tables['low_stock_skus']->isEmpty())
                ...
                @else
                    <table class="w-full text-left text-sm whitespace-nowrap">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @foreach ($tables['low_stock_skus'] as $sku)
                                <tr class="hover:bg-red-50/30 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span class="px-2.5 py-1 rounded-md text-xs font-bold {{ $sku->current_stock <= 0 ? 'bg-red-100 text-red-700' : 'bg-cyan-100 text-cyan-700' }}">
