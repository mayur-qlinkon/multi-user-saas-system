            ...
            <table class="w-full">
                ...
                <tbody>
                    @forelse($departments as $dept)
                        <tr class="table-row" x-show="matchesSearch('{{ strtolower($dept->name) }}', '{{ strtolower($dept->code ?? '') }}')">
                            <td class="px-5 py-3 text-[12px] font-bold text-gray-400">{{ $departments->firstItem() + $loop->index }}</td>
                            <td class="px-5 py-3">
                                <div>
                                    <p class="text-[13px] font-bold text-gray-800">{{ $dept->name }}</p>
                                    @if($dept->description)
                                        <p class="text-[11px] text-gray-400 mt-0.5 truncate max-w-[250px]">{{ $dept->description }}</p>
                                    @endif
                                </div>
                            </td>
                            <td class="px-3 py-3">
                                @if($dept->code)
                                    <span class="text-[11px] font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded">{{ $dept->code }}</span>
                                @else
