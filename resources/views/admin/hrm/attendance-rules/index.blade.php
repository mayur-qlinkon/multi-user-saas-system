                        ...
                        <tr class="table-row" x-show="matchesSearch('{{ strtolower($rule->name) }}')">
                            ...
                            <td class="px-5 py-3">
                                <div>
                                    ...
                                    @if($rule->description)
                                        <p class="text-[11px] text-gray-400 mt-0.5 truncate max-w-[250px]">{{ $rule->description }}</p>
                                    @endif
                                </div>
                            </td>
                            <td class="px-3 py-3">
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <select x-model="form.rule_type" class="field-input" required>
                            ...
                            @foreach(\App\Models\Hrm\AttendanceRule::TYPE_LABELS as $val => $label)
...
window.attendanceRulePage = function() {
    return {
        ...
        async submitForm() {
            ...
            const url = this.isEditing ? `{{ url('admin/hrm/attendance-rules') }}/${this.editId}` : `{{ route('admin.hrm.attendance-rules.store') }}`;
            try {
        ...
        confirmDelete(id, name) {
            BizAlert.confirm('Delete Rule', `Delete "${name}"?`, 'Delete').then(async (r) => {
                ...
                try {
                    const resp = await fetch(`{{ url('admin/hrm/attendance-rules') }}/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
                    const data = await resp.json();
