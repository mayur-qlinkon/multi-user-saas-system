...
@section('content')
    <div class="w-full px-4 py-6 sm:px-6 lg:px-8 xl:px-5" x-data="expenseEditForm(@js($expense), @js(old()))">
        ...
        {{-- ── Main Form ── --}}
        <form action="{{ route('admin.expenses.update', $expense) }}" method="POST" enctype="multipart/form-data" @submit="submitForm($event)">
            @csrf
            @method('PUT')
                                    ...
                                    <select name="expense_category_id" x-model="form.expense_category_id" required
                                        ...
                                        @foreach($categories as $parent)
                                            <optgroup label="{{ $parent->name }}">
                                                <option value="{{ $parent->id }}" @selected(old('expense_category_id', $expense->expense_category_id) == $parent->id)>{{ $parent->name }} (Main)</option>
                                                @foreach($parent->children as $child)
                                                    <option value="{{ $child->id }}" @selected(old('expense_category_id', $expense->expense_category_id) == $child->id)>&nbsp;&nbsp;&nbsp;↳ {{ $child->name }}</option>
                                                @endforeach
                                            </optgroup>
                                        @endforeach
                                    </select>
