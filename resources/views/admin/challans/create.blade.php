...
@section('title', 'Create Challan - Qlinkon BIZNESS')
...
@section('content')
    <div class="pb-20" x-data="challanForm(@js($units ?? []), @js($clients ?? []), @js($suppliers ?? []), @js($stores ?? []),@js($warehouses ?? []))">
        ...
        <form id="mainChallanForm" action="{{ route('admin.challans.store') }}" method="POST"
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    {{-- 🌟 FIX 2: Added Warehouse Selection --}}
                    <div>
                    ...
                    {{-- 🛰️ DESTINATION WAREHOUSE (Only for Branch Transfers) --}}
                    <div x-show="formData.challan_type === 'branch_transfer'" 
                        ...
                        <select name="to_warehouse_id" 
                            ...
                            @foreach ($warehouses ?? [] as $warehouse)
                                <option value="{{ $warehouse->id }}">{{ $warehouse->name }} ({{ $warehouse->store?->name }})</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    {{-- DYNAMIC SELECTOR (Clients / Suppliers / Stores) --}}                    
    ...
    <script>
        function challanForm(allUnits = [], allClients = [], allSuppliers = [], allStores = [],allWarehouses = []) {
            return {
                ...
                storeDefaults: @json(collect($warehouses ?? [])->where('is_default', 1)->pluck('id', 'store_id')),
                ...
                formData: {
                    challan_type: "{{ old('challan_type', '') }}", // 🌟 Wrap in old()
                    status: "{{ old('status', 'draft') }}",
                    direction: "{{ old('direction', 'outward') }}",
                    challan_date: "{{ old('challan_date', now()->format('Y-m-d')) }}",
                    store_id: "{{ old('store_id', session('store_id', auth()->user()->store_id ?? '')) }}",
                    warehouse_id: "{{ old('warehouse_id', '') }}",
                    to_warehouse_id: "{{ old('to_warehouse_id', '') }}", // 🌟 Bind to old value
                    
                    party_id: "{{ old('party_id', '') }}",
                    party_name: "{{ old('party_name', '') }}",
                    party_gst: "{{ old('party_gst', '') }}",
                    to_state_id: "{{ old('to_state_id', '') }}",
                    is_returnable: {{ old('is_returnable') ? 'true' : 'false' }},
                },
