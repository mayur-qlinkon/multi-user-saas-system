...
<div class="w-full" x-data="bulkImport({ productLimit: {{ $productLimit ?? 'null' }}, productCount: {{ $productCount }}, limitExceeded: {{ $productLimitReached ? 'true' : 'false' }} })" x-cloak>
    ...
    <div x-show="activeTab === 'products'">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6" x-show="!importId">
                ...
                {{-- Plan Limit: already at limit on page load ── --}}
                <div x-show="limitExceeded && !limitExceededDetails" x-transition
                ...
                {{-- Drop Zone (disabled when limit is exceeded) ── --}}
                ...
                {{-- Start Import button / Limit-blocked button ── --}}
                <div class="mt-4 flex justify-end" x-show="selectedFile && !uploading">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6" x-show="!importId">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('admin.bulk-import.sample', 'clients') }}"
                        target="_blank"
                        class="text-xs font-bold px-3 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors flex items-center gap-1.5">
                        <i data-lucide="download" class="w-3 h-3"></i> Sample CSV
                    </a>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="p-6" x-show="!importId">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('admin.bulk-import.sample', 'suppliers') }}"
                        target="_blank"
                        class="text-xs font-bold px-3 py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors flex items-center gap-1.5">
                        <i data-lucide="download" class="w-3 h-3"></i> Sample CSV
                    </a>
                    ...
                    <tr class="text-[12px]">
                        ...
                        <td class="px-4 py-2.5">
                            @if($imp->failed_rows > 0)
                                <a href="{{ route('admin.bulk-import.errors', $imp) }}" target="_blank" class="text-red-500 hover:text-red-700 text-[11px] font-bold">
                                    Errors
                                </a>
                            @endif
                        </td>
...
function bulkImport(config = {}) {
    return {
        ...
        tabs: [
            ...
            @if(has_permission('images_import'))
