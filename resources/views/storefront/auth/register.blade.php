...
<div class="flex-1 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-gray-50/50">
    <div class="max-w-md w-full bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <form action="{{ route('storefront.register.submit', ['slug' => $company->slug]) }}" method="POST" class="space-y-4">
            @csrf
            ...
            <div class="grid grid-cols-2 gap-4 pt-1">
                <div>
                    ...
                    <input type="password" name="password" id="password" required
                        class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all outline-none @error('password') border-red-300 @enderror">
                </div>
                <div>
        ...
        <p class="mt-8 text-center text-sm text-gray-600">
            ...
            <a href="{{ route('storefront.login', ['slug' => $company->slug]) }}" class="font-bold text-brand-600 hover:text-brand-700 transition-colors">Log in</a>
        </p>
    </div>
</div>
@endsection