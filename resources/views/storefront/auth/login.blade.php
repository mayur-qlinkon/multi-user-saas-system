@extends('layouts.storefront') {{-- Ensure this points to your main storefront layout --}}
...
<div class="flex-1 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-gray-50/50">
    <div class="max-w-md w-full bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-sm text-gray-500 mt-2">Log in to manage your orders at <span class="font-bold text-gray-800">{{ $company->name }}</span></p>
        </div>
        ...
        <form action="{{ route('storefront.login.submit', ['slug' => $company->slug]) }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                @error('email')
                    <p class="text-[11px] font-semibold text-red-500 mt-1.5">{{ $message }}</p>
                @enderror
            </div>
        ...
        <p class="mt-8 text-center text-sm text-gray-600">
            ...
            <a href="{{ route('storefront.register', ['slug' => $company->slug]) }}" class="font-bold text-brand-600 hover:text-brand-700 transition-colors">Create one</a>
        </p>
    </div>
</div>
@endsection