            ...
            >
                ...
                <div class="mx-5 mb-3 grid grid-cols-3 rounded-xl border border-gray-100 bg-gray-50/80 divide-x divide-gray-100 overflow-hidden">
                    <div class="px-3 py-2.5 text-center">
                        ...
                        <p class="text-sm font-semibold text-gray-700 mt-0.5 tabular-nums">{{ $currency }}{{ number_format((float)$totalAmount, 2) }}</p>
                    </div>
                    <div class="px-3 py-2.5 text-center">
                        <p class="text-[9px] font-semibold text-gray-400 uppercase tracking-widest">Paid</p>
                        <p class="text-sm font-semibold text-emerald-600 mt-0.5 tabular-nums">{{ $currency }}{{ number_format((float)$paidAmount, 2) }}</p>
                    </div>
                    <div class="px-3 py-2.5 text-center">
                        ...
                        >{{ $currency }}<span x-text="dueAmount.toFixed(2)"></span></p>
                    </div>
                </div>
                ...
                <form x-on:submit.prevent="submit()" class="px-5 pb-5 space-y-4" novalidate>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 text-sm select-none">{{ $currency }}</span>
                            <input
                                type="number"
                        ...
                        >
                            Balance remaining: <span class="font-semibold text-gray-600 tabular-nums" x-text="'{{ $currency }}' + Math.max(0, dueAmount - (parseFloat(form.amount) || 0)).toFixed(2)"></span>
                        </p>
                    </div>
                            ...
                            @forelse($paymentMethods as $method)
                                ...
                                >
                                    ...
                                    <span
                                        ...
                                        :class="form.payment_method_id == {{ $method->id }} ? 'bg-indigo-500' : 'bg-gray-300'"
                                    ></span>
                                    {{ $method->label }}
                                </label>
                            @empty
...
{{-- ══════════════════════════════════════════
     ALPINE.JS CONTROLLER — loaded once globally
══════════════════════════════════════════ --}}
@once
@push('scripts')
<script>
