...
<div class="pb-10" x-data="workLogPage()" x-init="init()">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <div class="stat-card">
            ...
            <p class="text-2xl font-black text-gray-900"><?php echo e(number_format($logs->getCollection()->sum('hours_worked'), 1)); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Approved</p>
            <p class="text-2xl font-black text-green-600"><?php echo e($logs->getCollection()->where('status', 'approved')->count()); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Pending</p>
            <p class="text-2xl font-black text-blue-500"><?php echo e($logs->getCollection()->where('status', 'submitted')->count()); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Rejected</p>
            <p class="text-2xl font-black text-red-500"><?php echo e($logs->getCollection()->where('status', 'rejected')->count()); ?></p>
        </div>
    </div>

    
    <div class="bg-white border border-gray-100 rounded-2xl px-4 py-3 mb-4">
        <form method="GET" action="<?php echo e(route('admin.hrm.work-logs.index')); ?>">
            ...
            <table class="w-full">
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $logs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        ?>
                        <tr class="table-row">
                            <td class="px-5 py-3 text-[12px] font-bold text-gray-400"><?php echo e($logs->firstItem() + $loop->index); ?></td>
                            <td class="px-5 py-3">
                                <div>
                                    <p class="text-[13px] font-bold text-gray-800"><?php echo e($log->employee?->user?->name ?? '—'); ?></p>
                                    <p class="text-[11px] text-gray-400"><?php echo e($log->employee?->employee_code); ?></p>
                                </div>
                            </td>
                            <td class="px-3 py-3 text-[12px] text-gray-600"><?php echo e($log->log_date ? \Carbon\Carbon::parse($log->log_date)->format('d M Y') : '—'); ?></td>
                            <td class="px-3 py-3 text-[12px] text-gray-600"><?php echo e($log->task?->title ?? '—'); ?></td>
                            <td class="px-3 py-3">
                                <p class="text-[12px] text-gray-600 truncate max-w-[180px]" title="<?php echo e($log->description); ?>">
                                    <?php echo e($log->description ?? '—'); ?>

                                </p>
                            </td>
                            <td class="px-3 py-3 text-center">
                                <span class="text-[13px] font-black text-gray-800"><?php echo e(number_format($log->hours_worked, 1)); ?></span>
                                <span class="text-[10px] text-gray-400 ml-0.5">h</span>
                            </td>
                            <td class="px-3 py-3 text-[12px] text-gray-600 capitalize"><?php echo e($log->category ?? '—'); ?></td>
                            <td class="px-3 py-3 text-center">
                                <span class="inline-flex items-center text-[10px] font-extrabold uppercase tracking-wider border px-2.5 py-1 rounded-md <?php echo e($statusBadge); ?>">
                                    <?php echo e($statusLabels[$log->status] ?? $log->status); ?>

                                </span>
                            </td>
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <?php if($log->status === 'submitted' && has_permission('work_logs.approve')): ?>
                                        <button @click="approveLog(<?php echo e($log->id); ?>)"
                                        ...
                                        <button @click="rejectLog(<?php echo e($log->id); ?>)"
...
window.workLogPage = function() {
    return {
        ...
        async approveLog(id) {
            try {
                const response = await fetch(`<?php echo e(url('admin/hrm/work-logs')); ?>/${id}/approve`, {
                    method: 'PATCH',
                    headers: {
        ...
        async submitReject() {
            this.saving = true;
            try {
                const response = await fetch(`<?php echo e(url('admin/hrm/work-logs')); ?>/${this.rejectId}/approve`, {
                    method: 'PATCH',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
        ...
        confirmDelete(id) {
            BizAlert.confirm('Delete Work Log', 'Are you sure you want to delete this work log?', 'Delete').then(async (result) => {
                ...
                try {
                    const response = await fetch(`<?php echo e(url('admin/hrm/work-logs')); ?>/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                            'Accept': 'application/json',
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlinkonSoftware\resources\views/admin/hrm/work-logs/index.blade.php ENDPATH**/ ?>