...
<?php $__env->startSection('header-title'); ?>
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-gray-400 font-medium mt-0.5"><?php echo e($leave->employee->user?->name); ?> · <?php echo e($leave->leaveType?->name); ?></p>
        </div>
    </div>
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-start justify-between gap-4 flex-wrap">
                        <div>
                            ...
                            <p class="text-[12px] text-gray-500 mt-0.5">
                                <?php echo e($leave->employee->employee_code); ?> &nbsp;·&nbsp;
                                <?php echo e($leave->employee->designation?->name); ?> &nbsp;·&nbsp;
                                <?php echo e($leave->employee->department?->name); ?>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
                        <div class="bg-gray-50 rounded-xl px-4 py-3">
                            ...
                            <p class="text-[13px] font-black text-gray-800"><?php echo e($leave->leaveType?->name); ?></p>
                        </div>
                        <div class="bg-gray-50 rounded-xl px-4 py-3">
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Duration</p>
                            <p class="text-[13px] font-black text-gray-800"><?php echo e($leave->total_days); ?> <?php echo e($leave->total_days == 1 ? 'day' : 'days'); ?></p>
                        </div>
                        <div class="bg-gray-50 rounded-xl px-4 py-3">
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">From</p>
                            <p class="text-[13px] font-black text-gray-800"><?php echo e($leave->from_date->format('d M Y')); ?></p>
                        </div>
                        <div class="bg-gray-50 rounded-xl px-4 py-3">
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">To</p>
                            <p class="text-[13px] font-black text-gray-800"><?php echo e($leave->to_date->format('d M Y')); ?></p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="detail-card">
                ...
                <div class="card-body">
                    <p class="text-[13px] text-gray-700 leading-relaxed"><?php echo e($leave->reason); ?></p>
                </div>
            </div>
            ...
            <div class="detail-card">
                ...
                <div class="card-body">
                    <a href="<?php echo e(Storage::url($leave->document)); ?>" target="_blank"
            ...
            <div class="detail-card">
                ...
                <div class="card-body">
                    <p class="text-[13px] text-gray-700 leading-relaxed"><?php echo e($leave->admin_remarks); ?></p>
                </div>
            </div>
            <?php endif; ?>
                ...
                <div class="card-body space-y-0">
                    ...
                    <div class="flex gap-3">
                        ...
                        <div class="pb-4">
                            ...
                            <p class="text-[11px] text-gray-400"><?php echo e($leave->created_at->format('d M Y, h:i A')); ?></p>
                        </div>
                    </div>
                    ...
                    <?php if($leave->approved_at): ?>
                        <div class="flex gap-3">
                            <div class="flex flex-col items-center">
                                <div class="timeline-dot <?php echo e($leave->status === 'approved' ? 'bg-green-500' : 'bg-red-500'); ?>"></div>
                                <?php if($leave->cancelled_at): ?> <div class="timeline-line"></div> <?php endif; ?>
                            </div>
                            <div class="<?php echo e($leave->cancelled_at ? 'pb-4' : ''); ?>">
                                <p class="text-[12px] font-bold text-gray-800 capitalize"><?php echo e($leave->status); ?></p>
                                <p class="text-[11px] text-gray-400"><?php echo e($leave->approved_at->format('d M Y, h:i A')); ?></p>
                                <?php if($leave->approvedByUser): ?>
                                    <p class="text-[11px] text-gray-500 mt-0.5">by <?php echo e($leave->approvedByUser->name); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    ...
                    <?php if($leave->cancelled_at): ?>
                        <div class="flex gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-gray-400"><?php echo e($leave->cancelled_at->format('d M Y, h:i A')); ?></p>
                                <?php if($leave->cancellation_reason): ?>
                                    <p class="text-[11px] text-gray-500 mt-0.5"><?php echo e($leave->cancellation_reason); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endif; ?>
            ...
            <div class="detail-card">
                ...
                <div class="card-body">
                    <div class="info-row">
                        ...
                        <span class="info-value capitalize"><?php echo e(str_replace('_', ' ', $leave->day_type)); ?></span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Paid Leave</span>
                        <span class="info-value"><?php echo e($leave->leaveType?->is_paid ? 'Yes' : 'No'); ?></span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Applied On</span>
                        <span class="info-value"><?php echo e($leave->created_at->format('d M Y')); ?></span>
                    </div>
                </div>
            </div>
        </div>
...
window.leaveShow = function() {
    return {
        ...
        async approveLeave() {
            this.saving = true;
            try {
                const resp = await fetch('<?php echo e(route("admin.hrm.leaves.approve", $leave)); ?>', {
                    method: 'PATCH',
        ...
        rejectLeave() {
            BizAlert.confirm('Reject Leave', 'Are you sure you want to reject this leave request?', 'Reject').then(async (r) => {
                ...
                this.saving = true;
                try {
                    const resp = await fetch('<?php echo e(route("admin.hrm.leaves.reject", $leave)); ?>', {
                        method: 'PATCH',
                        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                        body: JSON.stringify({ remarks: this.remarks }),
                    });
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlink-saas - Slug Based\resources\views/admin/hrm/leaves/show.blade.php ENDPATH**/ ?>