...
<div class="pb-12" x-data="announcementIndex()">
    ...
    <div class="bg-white rounded-t-xl shadow-sm border border-gray-100 p-4 border-b-0 mb-0">
        <form id="filterForm" method="GET" action="<?php echo e(route('admin.hrm.announcements.index')); ?>" class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2 w-full md:w-auto justify-start md:justify-end shrink-0">
                <?php if(has_permission('announcements.create')): ?>
                            ...
                            <td class="table-cell text-right">
                                <div class="flex items-center justify-end gap-2 transition-opacity">
                                    ...
                                    <?php if(!$announcement->is_published): ?>
                                        <?php if(has_permission('announcements.publish')): ?>
                                    ...
                                    <?php if(has_permission('announcements.update')): ?>
                                    ...
                                    <?php if(has_permission('announcements.create')): ?>
                                        <button @click="duplicateAnnouncement(<?php echo e($announcement->id); ?>, '<?php echo e(addslashes($announcement->title)); ?>')" title="Duplicate as Draft"
                                    ...
                                    <?php if(has_permission('announcements.delete')): ?>
    ...
    return {
        ...
        duplicateAnnouncement(id, title) {
            BizAlert.confirm('Duplicate Announcement', `Duplicate "${title}" as a new draft?`, 'Yes, Duplicate').then(async r => {
                ...
                try {
                    const response = await fetch(`<?php echo e(url('admin/hrm/announcements')); ?>/${id}/duplicate`, {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                            'Accept': 'application/json',
