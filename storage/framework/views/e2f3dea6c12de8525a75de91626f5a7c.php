...
window.attendanceSettings = function() {
    return {
        ...
        errors: {},
        form: {
            attendance_checkin_start: <?php echo json_encode($settings['attendance_checkin_start'] ?? '08:00', 15, 512) ?>,
            attendance_checkin_end: <?php echo json_encode($settings['attendance_checkin_end'] ?? '10:30', 15, 512) ?>,
            attendance_checkout_time: <?php echo json_encode($settings['attendance_checkout_time'] ?? '17:00', 15, 512) ?>,
            attendance_late_threshold_minutes: <?php echo json_encode($settings['attendance_late_threshold_minutes'] ?? '15', 15, 512) ?>,
            attendance_min_working_hours: <?php echo json_encode($settings['attendance_min_working_hours'] ?? '8', 15, 512) ?>,
            attendance_half_day_hours: <?php echo json_encode($settings['attendance_half_day_hours'] ?? '4', 15, 512) ?>,
            attendance_qr_expiry_seconds: <?php echo json_encode($settings['attendance_qr_expiry_seconds'] ?? '30', 15, 512) ?>,
            attendance_scan_cooldown_seconds: <?php echo json_encode($settings['attendance_scan_cooldown_seconds'] ?? '60', 15, 512) ?>,
            attendance_gps_radius_meters: <?php echo json_encode($settings['attendance_gps_radius_meters'] ?? '100', 15, 512) ?>,
            attendance_office_lat: <?php echo json_encode($settings['attendance_office_lat'] ?? '', 15, 512) ?>,
            attendance_office_lng: <?php echo json_encode($settings['attendance_office_lng'] ?? '', 15, 512) ?>,
        },
        ...
        async saveSettings() {
            ...
            try {
                const res = await fetch(`<?php echo e(route('admin.hrm.attendance-settings.update')); ?>`, {
                    method: 'PUT',
                    headers: {
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlinkonSoftware\resources\views/admin/hrm/attendance/settings.blade.php ENDPATH**/ ?>