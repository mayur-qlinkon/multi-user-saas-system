...
<div class="pb-10" x-data="designationPage()">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-5">
        <div class="stat-card">
            ...
            <p class="text-2xl font-black text-gray-900"><?php echo e($designations->total()); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Active</p>
            <p class="text-2xl font-black text-green-600"><?php echo e($designations->where('is_active', true)->count()); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Inactive</p>
            <p class="text-2xl font-black text-gray-400"><?php echo e($designations->where('is_active', false)->count()); ?></p>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl px-4 py-3 mb-4">
        <div class="flex items-center gap-3 flex-wrap">
            ...
            <?php if(has_permission('designations.create')): ?>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full block md:table">
                ...
                <tbody class="block md:table-row-group p-4 md:p-0">
                    <?php $__empty_1 = true; $__currentLoopData = $designations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $desig): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="block md:table-row bg-white border border-gray-200 md:border-0 md:border-b md:border-gray-100 rounded-xl md:rounded-none mb-4 md:mb-0 p-4 md:p-0 shadow-sm md:shadow-none" x-show="matchesSearch('<?php echo e(strtolower($desig->name)); ?>')">
                            <td class="hidden md:table-cell px-5 py-3 text-[12px] font-bold text-gray-400"><?php echo e($designations->firstItem() + $loop->index); ?></td>
                            <td class="block md:table-cell md:px-5 md:py-3 mb-3 md:mb-0">
                                <p class="text-[13px] font-bold text-gray-800"><?php echo e($desig->name); ?></p>
                                <?php if($desig->description): ?>
                                    <p class="text-[11px] text-gray-400 mt-0.5 truncate max-w-[300px]"><?php echo e($desig->description); ?></p>
                                <?php endif; ?>
                            </td>
                            <td class="flex justify-between items-center md:table-cell md:px-3 md:py-3 mb-2 md:mb-0 md:text-center border-b border-gray-50 md:border-none pb-2 md:pb-0">
                                ...
                                <span class="text-[11px] font-bold text-gray-500 bg-gray-100 px-2.5 py-0.5 rounded">Level <?php echo e($desig->level); ?></span>
                            </td>
                            ...
                            <td class="flex justify-end items-center md:table-cell pt-3 md:pt-0 border-t border-gray-100 md:border-none md:px-4 md:py-3 md:text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <?php if(has_permission('designations.update')): ?>
                                        <button @click="openEdit(<?php echo e($desig->toJson()); ?>)"
                                    ...
                                    <?php endif; ?>
                                    <?php if(has_permission('designations.delete')): ?>
                                        <button @click="confirmDelete(<?php echo e($desig->id); ?>, '<?php echo e($desig->name); ?>')"
        ...
        <?php if($designations->hasPages()): ?>
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50"><?php echo e($designations->links()); ?></div>
        <?php endif; ?>
    </div>
...
window.designationPage = function() {
    return {
        ...
        async submitForm() {
            ...
            const url = this.isEditing ? `<?php echo e(url('admin/hrm/designations')); ?>/${this.editId}` : `<?php echo e(route('admin.hrm.designations.store')); ?>`;
            try {
        ...
        confirmDelete(id, name) {
            BizAlert.confirm('Delete Designation', `Delete "${name}"?`, 'Delete').then(async (r) => {
                ...
                try {
                    const resp = await fetch(`<?php echo e(url('admin/hrm/designations')); ?>/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
                    const data = await resp.json();
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlinkonSoftware\resources\views/admin/hrm/designations/index.blade.php ENDPATH**/ ?>