                    ...
                    <div class="p-4 hover:bg-gray-50/50 transition-colors flex flex-col gap-3 group">
                        ...
                        <div class="flex justify-between items-start gap-2">
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <div class="text-[14px] font-bold text-gray-900 truncate"><?php echo e($user->name); ?></div>
                                    <div class="text-[11px] text-gray-500 mt-0.5 truncate"><?php echo e($user->email); ?></div>
                                </div>
                            </div>
                                        ...
                                        <div class="flex flex-wrap justify-end gap-1.5">
                                            <?php $__currentLoopData = $user->stores->take(2); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $store): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <span class="inline-flex items-center gap-1 bg-white text-gray-600 border border-gray-200 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider shadow-sm">
                                                    <i data-lucide="store" class="w-2.5 h-2.5 text-brand-500"></i> <?php echo e($store->name); ?>

                                                </span>
                                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                            <?php if($userStoreCount > 2): ?>
                                                <?php $hiddenStores = $user->stores->skip(2)->pluck('name')->implode(', '); ?>
                                                <span title="Also assigned to: <?php echo e($hiddenStores); ?>" class="inline-flex items-center text-[9px] font-black text-brand-600 bg-brand-50 px-1.5 py-0.5 rounded border border-brand-100">
                                                    +<?php echo e($userStoreCount - 2); ?> More
                                                </span>
                                            <?php endif; ?>
                                        </div>
                        ...
                        <div class="flex items-center justify-end gap-2 pt-1 border-t border-gray-50 mt-1">
                            <?php if(has_permission('users.view')): ?>
                                <a href="<?php echo e(route('admin.users.show', $user->id)); ?>" class="w-8 h-8 rounded-lg bg-gray-50 text-gray-600 hover:bg-gray-200 hover:text-gray-800 flex items-center justify-center transition-colors" title="View Staff Details">
                                    <i data-lucide="eye" class="w-4 h-4"></i>
                                </a>
                            <?php endif; ?>
                            ...
                            <?php if(auth()->id() !== $user->id && ($user->roles->first()->slug ?? '') !== 'owner'): ?>
                                <?php if(has_permission('users.update')): ?>
                                    <a href="<?php echo e(route('admin.users.edit', $user->id)); ?>" class="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 hover:text-blue-700 flex items-center justify-center transition-colors" title="Edit Staff">
                                        <i data-lucide="pencil" class="w-4 h-4"></i>
                                    </a>
                                <?php endif; ?>

                                <?php if(has_permission('users.delete')): ?>
                                    <form action="<?php echo e(route('admin.users.destroy', $user->id)); ?>" method="POST" class="inline-block delete-staff-form" data-name="<?php echo e(addslashes($user->name)); ?>">
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlink-saas - Slug Based\resources\views/admin/users/index.blade.php ENDPATH**/ ?>