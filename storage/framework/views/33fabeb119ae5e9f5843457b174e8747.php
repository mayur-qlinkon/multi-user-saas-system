                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-5 py-3 w-full md:w-auto">
                                        ...
                                        <div class="flex items-center flex-wrap gap-2 mt-1.5">
                                            ...
                                            <span class="text-[10px] font-bold text-gray-400 uppercase bg-gray-50 border border-gray-200 px-1.5 py-0.5 rounded"
                        ...
                        <div class="flex flex-col gap-3 p-4 bg-white relative">
                            ...
                            <div class="flex justify-between items-start pr-8">
                                <div>
                                    ...
                                    <span x-show="item.discount_value > 0"
                                        class="bg-amber-100 text-amber-700 text-[10px] px-1.5 py-0.5 rounded font-bold mt-1.5 inline-block border border-amber-200">
    ...
    <script>
        function invoiceForm(allUnits = [], companyState = '', invoiceData = null, allClients = []) {
            ...
            return {
                ...
                formData: {
                    customer_id: <?php echo json_encode(old('customer_id'), 15, 512) ?> || (invoiceData ? (invoiceData.customer_id || '') : ''),
                    customer_name: <?php echo json_encode(old('customer_name'), 15, 512) ?> || (invoiceData ? (invoiceData.customer_name || '') : ''),
                    customer_gstin: invoiceData && invoiceData.client ? (invoiceData.client.gst_number || '') : '',
                    gst_treatment: <?php echo json_encode(old('gst_treatment'), 15, 512) ?> || (invoiceData ? (invoiceData.gst_treatment || 'unregistered') : 'unregistered'),
                    supply_state: <?php echo json_encode(old('supply_state'), 15, 512) ?> || (invoiceData ? invoiceData.supply_state : companyState),
                    payment_method_id: <?php echo json_encode(old('payment_method_id'), 15, 512) ?> || (initialPayment ? initialPayment.payment_method_id : ''),
                    invoice_date: <?php echo json_encode(old('invoice_date'), 15, 512) ?> || (invoiceData ? invoiceData.invoice_date.split('T')[0] : new Date().toISOString().split('T')[0]),
                    due_date: <?php echo json_encode(old('due_date'), 15, 512) ?> || (invoiceData && invoiceData.due_date ? invoiceData.due_date.split('T')[0] : new Date(new Date().setDate(new Date().getDate() + 7)).toISOString().split('T')[0]),
                    store_id: <?php echo json_encode(old('store_id'), 15, 512) ?> || (invoiceData ? invoiceData.store_id : "<?php echo e(auth()->user()->store_id ?? ''); ?>"),
                    warehouse_id: <?php echo json_encode(old('warehouse_id'), 15, 512) ?> || (invoiceData ? invoiceData.warehouse_id : ''),
                },

                global: {
                    shipping: parseFloat(<?php echo json_encode(old('shipping_charge'), 15, 512) ?>) || (invoiceData ? parseFloat(invoiceData.shipping_charge) || 0 : 0),
                    amount_paid: parseFloat(<?php echo json_encode(old('amount_paid'), 15, 512) ?>) || (initialPayment ? parseFloat(initialPayment.amount_received || initialPayment.amount) : 0),
                    discount_type: <?php echo json_encode(old('discount_type'), 15, 512) ?> || ((invoiceData && invoiceData.discount_type) ? (invoiceData.discount_type === 'percentage' ? 'percent' : invoiceData.discount_type) : 'fixed'),
                    discount_value: parseFloat(<?php echo json_encode(old('discount_value'), 15, 512) ?>) || (invoiceData ? parseFloat(invoiceData.discount_amount) || 0 : 0),
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlink-saas - Slug Based\resources\views/admin/invoices/edit.blade.php ENDPATH**/ ?>