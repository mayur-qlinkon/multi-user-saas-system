...
<?php $__env->startSection('header-title'); ?>
    <h1 class="text-sm font-bold text-gray-500 uppercase tracking-widest">Edit Purchase
        <?php echo e($purchase->purchase_number); ?></h1>
...
<?php $__env->startSection('content'); ?>
    <div class="pb-10" x-data="purchaseForm(<?php echo \Illuminate\Support\Js::from($units ?? [])->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($purchase)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($purchase->items)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($batchEnabled ?? false)->toHtml() ?>)">
        <div class="mb-6">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <select name="store_id" required
                            ...
                            <?php $__currentLoopData = $stores ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $store): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($store->id); ?>"
                                    <?php echo e(old('store_id', $purchase->store_id) == $store->id ? 'selected' : ''); ?>>
                    ...
                    <div>
                        ...
                        <input type="date" name="due_date"
                            value="<?php echo e(old('due_date', $purchase->due_date?->format('Y-m-d'))); ?>"
                            class="w-full border border-gray-300 rounded px-3 py-2.5 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" required
                            class="w-full border border-gray-300 rounded px-3 py-2.5 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none bg-white">
                            <option value="draft" <?php echo e(old('status', $purchase->status) == 'draft' ? 'selected' : ''); ?>>
                                Draft</option>
                            <option value="ordered" <?php echo e(old('status', $purchase->status) == 'ordered' ? 'selected' : ''); ?>>
                                Ordered / Sent</option>
                            
                            <option value="received"
                                <?php echo e(old('status', $purchase->status) == 'received' ? 'selected' : ''); ?>>Received (Updates
                                Stock)</option>
                            <option value="cancelled"
                                <?php echo e(old('status', $purchase->status) == 'cancelled' ? 'selected' : ''); ?>>Cancelled</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="supplier_invoice_number"
                            value="<?php echo e(old('supplier_invoice_number', $purchase->supplier_invoice_number)); ?>"
                    ...
                    <div>
                        ...
                        <input type="date" name="supplier_invoice_date"
                            value="<?php echo e(old('supplier_invoice_date', $purchase->supplier_invoice_date)); ?>"
                            class="w-full border border-gray-300 rounded px-3 py-2.5 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none">
                    </div>
                </div>
            </div>
                            ...
                            <template x-for="result in globalSearchResults" :key="result.product_sku_id">
                                <li @click="addSkuToTable(result); showResults = false"
                                    ...
                                    <div class="flex justify-between items-start gap-2">
                                        ...
                                        <span x-text="'₹' + result.cost" class="text-[#108c2a] font-bold text-[12px] flex-shrink-0"></span>
                                    </div>
                ...
                <div class="lg:col-span-2 bg-white rounded-lg shadow-sm border border-gray-200 p-6 flex flex-col gap-4">
                    <div>
                        ...
                        <textarea name="notes" rows="3"
                            class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none resize-none"><?php echo e(old('notes', $purchase->notes)); ?></textarea>
                    </div>
                    <div>
                        ...
                        <textarea name="terms_and_conditions" rows="3"
                            class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none resize-none"><?php echo e(old('terms_and_conditions', $purchase->terms_and_conditions)); ?></textarea>
                    </div>
                </div>
...
<?php $__env->startPush('scripts'); ?>
    <script>
        function purchaseForm(allUnits = [], purchaseData = {}, dbItems = [], batchEnabled = false) {
            ...
            const oldItemsRaw = <?php echo json_encode(old('items'), 15, 512) ?>;
            let initialItems = [];
            ...
            return {
                ...
                global: {
                    discount_type: "<?php echo e(old('discount_type')); ?>" !== "" ? "<?php echo e(old('discount_type')); ?>" : (purchaseData.discount_type === 'fixed' ? 'fixed' : 'percent'),
                    discount_value: parseFloat("<?php echo e(old('discount_value')); ?>" !== "" ? "<?php echo e(old('discount_value')); ?>" : purchaseData.discount_value) || 0,
                    discount_amount: 0,
                    shipping: parseFloat("<?php echo e(old('shipping_cost')); ?>" !== "" ? "<?php echo e(old('shipping_cost')); ?>" : purchaseData
                        .shipping_cost) || 0,
                    other: parseFloat("<?php echo e(old('other_charges')); ?>" !== "" ? "<?php echo e(old('other_charges')); ?>" : purchaseData
                        .other_charges) || 0,
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlink-saas - Slug Based\resources\views/admin/purchases/edit.blade.php ENDPATH**/ ?>