...
<div class="pb-10" x-data="shiftPage()">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <div class="stat-card">
            ...
            <p class="text-2xl font-black text-gray-900"><?php echo e($shifts->total()); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Active</p>
            <p class="text-2xl font-black text-green-600"><?php echo e($shifts->where('is_active', true)->count()); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Night Shifts</p>
            <p class="text-2xl font-black text-indigo-600"><?php echo e($shifts->where('is_night_shift', true)->count()); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Employees</p>
            <p class="text-2xl font-black text-gray-900"><?php echo e($shifts->sum('employees_count')); ?></p>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl px-4 py-3 mb-4">
        <div class="flex items-center gap-3 flex-wrap">
            ...
            <?php if(has_permission('shifts.create')): ?>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl overflow-hidden">
            ...
            <table class="w-full block md:table">
                ...
                <tbody class="block md:table-row-group p-4 md:p-0">
                    <?php $__empty_1 = true; $__currentLoopData = $shifts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $shift): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="block md:table-row bg-white border border-gray-200 md:border-0 md:border-b md:border-gray-100 rounded-xl md:rounded-none mb-4 md:mb-0 p-4 md:p-0 shadow-sm md:shadow-none hover:bg-gray-50 transition-colors" x-show="matchesSearch('<?php echo e(strtolower($shift->name)); ?>')">
                            <td class="hidden md:table-cell px-5 py-3 text-[12px] font-bold text-gray-400"><?php echo e($shifts->firstItem() + $loop->index); ?></td>
                            <td class="block md:table-cell md:px-5 md:py-3 mb-3 md:mb-0">
                                <div class="flex items-center gap-2">
                                    <div>
                                        <p class="text-[13px] font-bold text-gray-800"><?php echo e($shift->name); ?></p>
                                        <?php if($shift->code): ?>
                                            <span class="text-[10px] font-bold text-gray-400"><?php echo e($shift->code); ?></span>
                                        <?php endif; ?>
                                    </div>
                            ...
                            <td class="flex justify-between items-center md:table-cell md:px-3 md:py-3 mb-2 md:mb-0 border-b border-gray-50 md:border-none pb-2 md:pb-0">
                                ...
                                <div class="text-right md:text-left">
                                    <p class="text-[12px] font-bold text-gray-700"><?php echo e($shift->formatted_timing); ?></p>
                                    <?php if($shift->late_mark_after): ?>
                                        <p class="text-[10px] text-gray-400">Late after <?php echo e(\Carbon\Carbon::parse($shift->late_mark_after)->format('h:i A')); ?></p>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td class="flex justify-between items-center md:table-cell md:px-3 md:py-3 mb-2 md:mb-0 md:text-center border-b border-gray-50 md:border-none pb-2 md:pb-0">
                                ...
                                <span class="text-[12px] text-gray-600"><?php echo e($shift->break_duration_minutes); ?> min</span>
                            </td>
                            <td class="flex justify-between items-center md:table-cell md:px-3 md:py-3 mb-2 md:mb-0 md:text-center border-b border-gray-50 md:border-none pb-2 md:pb-0">
                                <span class="md:hidden text-[11px] font-bold text-gray-400 uppercase tracking-wider">Employees</span>
                                <span class="text-[12px] font-bold text-gray-700"><?php echo e($shift->employees_count); ?></span>
                            </td>
                            <td class="flex justify-between items-center md:table-cell md:px-3 md:py-3 mb-2 md:mb-0 md:text-center border-b border-gray-50 md:border-none pb-2 md:pb-0">
                            ...
                            <td class="flex justify-end items-center md:table-cell pt-3 md:pt-0 border-t border-gray-100 md:border-none md:px-4 md:py-3 md:text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <?php if(has_permission('shifts.update')): ?>
                                    <button @click="openEdit(<?php echo e($shift->toJson()); ?>)"
                                    ...
                                    <?php endif; ?>
                                    <?php if(has_permission('shifts.delete')): ?>
                                    <button @click="confirmDelete(<?php echo e($shift->id); ?>, '<?php echo e($shift->name); ?>')"
        ...
        <?php if($shifts->hasPages()): ?>
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50"><?php echo e($shifts->links()); ?></div>
        <?php endif; ?>
    </div>
...
window.shiftPage = function() {
    return {
        ...
        async submitForm() {
            ...
            const url = this.isEditing ? `<?php echo e(url('admin/hrm/shifts')); ?>/${this.editId}` : `<?php echo e(route('admin.hrm.shifts.store')); ?>`;
            try {
        ...
        confirmDelete(id, name) {
            BizAlert.confirm('Delete Shift', `Delete "${name}"?`, 'Delete').then(async (r) => {
                ...
                try {
                    const resp = await fetch(`<?php echo e(url('admin/hrm/shifts')); ?>/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
                    const data = await resp.json();
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlink-saas - Slug Based\resources\views/admin/hrm/shifts/index.blade.php ENDPATH**/ ?>