...
<div class="pb-10" x-data="holidayPage()">
    ...
    <div class="bg-white border border-gray-100 rounded-2xl px-4 py-3 mb-4">
        <form method="GET" action="<?php echo e(route('admin.hrm.holidays.index')); ?>">
            <div class="flex items-center gap-3 flex-wrap">
                ...
                <select name="type" class="field-input !w-auto !py-2 !text-[13px]" onchange="this.form.submit()">
                    ...
                    <?php $__currentLoopData = \App\Models\Hrm\Holiday::TYPE_LABELS; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <?php if(request()->hasAny(['type'])): ?>
                    <a href="<?php echo e(route('admin.hrm.holidays.index', ['year' => request('year', now()->year)])); ?>"
            ...
            <table class="w-full block md:table">
                ...
                <tbody class="block md:table-row-group p-4 md:p-0">
                    <?php $__empty_1 = true; $__currentLoopData = $holidays; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $holiday): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        <tr class="block md:table-row bg-white border border-gray-200 md:border-0 md:border-b md:border-gray-100 rounded-xl md:rounded-none mb-4 md:mb-0 p-4 md:p-0 shadow-sm md:shadow-none hover:bg-gray-50 transition-colors" x-show="matchesSearch('<?php echo e(strtolower($holiday->name)); ?>')">
                            <td class="hidden md:table-cell px-5 py-3 text-[12px] font-bold text-gray-400"><?php echo e($holidays->firstItem() + $loop->index); ?></td>
                            <td class="block md:table-cell md:px-5 md:py-3 mb-3 md:mb-0">
                                <p class="text-[14px] md:text-[13px] font-bold text-gray-800"><?php echo e($holiday->name); ?></p>
                            ...
                            <td class="flex justify-between items-center md:table-cell md:px-3 md:py-3 mb-2 md:mb-0 md:text-center border-b border-gray-50 md:border-none pb-2 md:pb-0">
                                ...
                                <span class="text-[12px] text-gray-600"><?php echo e($holiday->date->format('l')); ?></span>
                            </td>
                            <td class="flex justify-between items-center md:table-cell md:px-3 md:py-3 mb-2 md:mb-0 md:text-center border-b border-gray-50 md:border-none pb-2 md:pb-0">
                            ...
                            <td class="flex justify-end items-center md:table-cell pt-3 md:pt-0 border-t border-gray-100 md:border-none md:px-4 md:py-3 md:text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    <button @click="confirmDelete(<?php echo e($holiday->id); ?>, '<?php echo e($holiday->name); ?>')"
                        ...
                        <tr>
                            <td colspan="7">
                                <div class="flex flex-col items-center justify-center py-20 text-center">
                                    ...
                                    <p class="text-sm text-gray-400 mb-4">Add public and company holidays for <?php echo e(request('year', now()->year)); ?></p>
...
window.holidayPage = function() {
    return {
        ...
        async submitForm() {
            ...
            const url = this.isEditing ? `<?php echo e(url('admin/hrm/holidays')); ?>/${this.editId}` : `<?php echo e(route('admin.hrm.holidays.store')); ?>`;
            try {
        ...
        confirmDelete(id, name) {
            BizAlert.confirm('Delete Holiday', `Delete "${name}"?`, 'Delete').then(async (r) => {
                ...
                try {
                    const resp = await fetch(`<?php echo e(url('admin/hrm/holidays')); ?>/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
                    const data = await resp.json();
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlinkonSoftware\resources\views/admin/hrm/holidays/index.blade.php ENDPATH**/ ?>