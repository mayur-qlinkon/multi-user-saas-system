...
<?php $__env->startSection('title', 'Manage Products — ' . $section->title); ?>
...
<div class="pb-10" x-data="sectionProducts()" x-init="init()">
    ...
    <div class="flex items-center gap-3 mb-5 text-sm text-gray-500 font-medium">
        ...
        <span>Max <?php echo e($section->products_limit); ?> shown on storefront</span>
        <template x-if="assignedProducts.length > <?php echo e($section->products_limit); ?>">
            <span class="text-amber-600 font-semibold flex items-center gap-1">
                <i data-lucide="alert-triangle" class="w-3.5 h-3.5"></i>
                Only first <?php echo e($section->products_limit); ?> will display
            </span>
        </template>
    </div>
...
function sectionProducts() {
    return {
        ...
        init() {
            ...
            console.log('[SectionProducts] Init section #<?php echo e($section->id); ?>');
        },
        ...
        async loadAssigned() {
            ...
            try {
                const res  = await fetch('<?php echo e(route('admin.storefront-sections.products.load', $section->id)); ?>', {
        ...
        async saveOrder() {
            ...
            try {
                const res  = await fetch('<?php echo e(route('admin.storefront-sections.products.reorder', $section->id)); ?>', {
                    method:  'POST',
                    headers: {
        ...
        async searchProducts() {
            ...
            this.searching = true;
            try {
                const url  = '<?php echo e(route('admin.storefront-sections.products.search', $section->id)); ?>?q=' + encodeURIComponent(this.searchQuery);
        ...
        async addProduct(product) {
            ...
            try {
                const res  = await fetch('<?php echo e(route('admin.storefront-sections.products.add', $section->id)); ?>', {
                    method:  'POST',
                    headers: {
                        'Content-Type':     'application/json',
                        'X-CSRF-TOKEN':     document.querySelector('meta[name="csrf-token"]').content,
        ...
        async removeProduct(productId) {
            ...
            try {
                const res  = await fetch(
                    `<?php echo e(url('admin/storefront-sections/' . $section->id . '/products')); ?>/${productId}`,
                    {
                        method:  'DELETE',
                        headers: {
                            'X-CSRF-TOKEN':     document.querySelector('meta[name="csrf-token"]').content,
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlinkonSoftware\resources\views/admin/storefront-sections/products.blade.php ENDPATH**/ ?>