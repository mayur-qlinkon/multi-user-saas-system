        ...
        <div class="lg:col-span-2 space-y-4 order-2 lg:order-1">
            ...
            <?php if($task->description): ?>
                <div class="info-card">
                    ...
                    <div class="card-body">
                        <script>window.__taskDesc = <?php echo json_encode($task->description, 15, 512) ?>;</script>
                        <div id="task-desc-rendered"
                ...
                <div class="card-body space-y-4">
                    ...
                    <div id="commentsList" class="space-y-4">
                        <?php $__empty_1 = true; $__currentLoopData = $task->comments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $comment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <div class="flex gap-3" id="comment-<?php echo e($comment->id); ?>">
                                <div class="user-avatar flex-shrink-0"><?php echo e(strtoupper(substr($comment->user->name ?? '?', 0, 1))); ?></div>
                                <div class="flex-1">
                                    <div class="comment-bubble">
                                        <div class="flex items-center justify-between mb-1.5">
                                            <span class="text-[12px] font-bold text-gray-800"><?php echo e($comment->user->name ?? 'Unknown'); ?></span>
                                            <span class="text-[10px] text-gray-400"><?php echo e($comment->created_at->diffForHumans()); ?></span>
                                        </div>
                                        <p class="text-[12.5px] text-gray-600 leading-relaxed"><?php echo e($comment->body); ?></p>
                                    </div>

                                    
                                    <?php if($comment->replies && $comment->replies->count()): ?>
                                        <div class="mt-2 ml-4 space-y-2">
                                            <?php $__currentLoopData = $comment->replies; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reply): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <div class="flex gap-2">
                                                    <div class="user-avatar !w-[26px] !h-[26px] !text-[10px] flex-shrink-0"><?php echo e(strtoupper(substr($reply->user->name ?? '?', 0, 1))); ?></div>
                                                    <div class="flex-1">
                                                        <div class="reply-bubble">
                                                            <div class="flex items-center justify-between mb-1">
                                                                <span class="text-[11px] font-bold text-gray-700"><?php echo e($reply->user->name ?? 'Unknown'); ?></span>
                                                                <span class="text-[10px] text-gray-400"><?php echo e($reply->created_at->diffForHumans()); ?></span>
                                                            </div>
                                                            <p class="text-[12px] text-gray-600"><?php echo e($reply->body); ?></p>
                                                        </div>
                                                    </div>
                                                </div>
                                    ...
                                    <button type="button"
                                        @click="replyTo = replyTo === <?php echo e($comment->id); ?> ? null : <?php echo e($comment->id); ?>"
                                    ...
                                    <div x-show="replyTo === <?php echo e($comment->id); ?>" x-cloak class="mt-2">
                                        <div class="flex gap-2">
                                            ...
                                            <button type="button"
                                                @click="submitComment(<?php echo e($task->id); ?>, <?php echo e($comment->id); ?>)"
                    ...
                    <div class="border-t border-gray-100 pt-4">
                        ...
                        <div class="flex justify-end mt-2">
                            <?php if(has_permission('hrm_tasks.add_comment')): ?>
                            <button type="button"
                                @click="submitComment(<?php echo e($task->id); ?>, null)"
...
window.taskShow = function() {
    return {
        ...
        async submitComment(taskId, parentId) {
            ...
            try {
                const res = await fetch(`<?php echo e(url('admin/hrm/tasks')); ?>/${taskId}/comments`, {
                    method: 'POST',
                    headers: {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* ── Render task description markdown ─── */
    const desc = document.getElementById('task-desc-rendered');
    if (desc) {
        desc.innerHTML = renderTaskMd(window.__taskDesc || '');
    }
});

function renderTaskMd(text) {
    ...
    lines.forEach(line => {
        if (/^- /.test(line)) {
            ...
            html += `<li style="margin:2px 0">${inlineTaskMd(line.slice(2))}</li>`;
        } else {
            if (inList) { html += '</ul>'; inList = false; }
            if (line.trim() === '') {
                html += '<div style="height:6px"></div>';
            } else {
                html += `<p style="margin:2px 0">${inlineTaskMd(line)}</p>`;
            }
        }
    });
    if (inList) html += '</ul>';
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlinkonSoftware\resources\views/admin/hrm/tasks/show.blade.php ENDPATH**/ ?>