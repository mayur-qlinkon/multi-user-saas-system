<?php $__env->startSection('title', 'Edit Quotation - ' . $quotation->quotation_number); ?>
...
<?php $__env->startSection('content'); ?>
    ...
    <div class="pb-20" x-data="quotationForm(<?php echo \Illuminate\Support\Js::from($units ?? [])->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($companyState ?? '')->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($clients ?? [])->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($quotation)->toHtml() ?>)">

        <div class="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-[1.5rem] font-bold text-[#212538] tracking-tight mb-1">
                    Edit Proposal: <span class="text-[#108c2a]"><?php echo e($quotation->quotation_number); ?></span>
                </h1>
            </div>
        </div>
        ...
        <form id="mainQuotationForm" action="<?php echo e(route('admin.quotations.update', $quotation->id)); ?>" method="POST"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 flex flex-col gap-4">
                    ...
                    <textarea name="notes" rows="3" placeholder="Add notes.."
                        class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-brand-500 outline-none resize-none"><?php echo e(old('notes', $quotation->notes)); ?></textarea>
                    <textarea name="terms_conditions" rows="4" placeholder="Terms and conditions for this proposal..."
                        class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-brand-500 outline-none resize-none"><?php echo e(old('terms_conditions', $quotation->terms_conditions)); ?></textarea>
                </div>
        ...
        function quotationForm(allUnits = [], companyState = '', allClients = [], existingData = null) {
            ...
            return {
                ...
                formData: {
                    ...
                    store_id: existingData?.store_id || "<?php echo e(auth()->user()->store_id ?? ''); ?>",
                ...
                addSkuToTable(result) {
                    ...
                    this.items.push({
                        ...
                        // 🌟 FIX: Safely parse dynamic tax using '??' so 0% tax doesn't trigger a fallback
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlink-saas - Slug Based\resources\views/admin/quotations/edit.blade.php ENDPATH**/ ?>