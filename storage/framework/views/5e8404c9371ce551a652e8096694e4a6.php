...
<div class="pb-10" x-data="leaveTypePage()">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <div class="stat-card">
            ...
            <p class="text-2xl font-black text-gray-900"><?php echo e($leaveTypes->total()); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Active</p>
            <p class="text-2xl font-black text-green-600"><?php echo e($leaveTypes->where('is_active', true)->count()); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Paid Types</p>
            <p class="text-2xl font-black text-blue-600"><?php echo e($leaveTypes->where('is_paid', true)->count()); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Unpaid Types</p>
            <p class="text-2xl font-black text-orange-600"><?php echo e($leaveTypes->where('is_paid', false)->count()); ?></p>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl overflow-hidden">
            ...
            <table class="w-full">
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $leaveTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $leaveType): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="table-row" x-show="matchesSearch('<?php echo e(strtolower($leaveType->name)); ?>')">
                            <td class="px-5 py-3 text-[12px] font-bold text-gray-400"><?php echo e($leaveTypes->firstItem() + $loop->index); ?></td>
                            <td class="px-5 py-3">
                                <div class="flex items-center gap-2">
                                    <div>
                                        <p class="text-[13px] font-bold text-gray-800"><?php echo e($leaveType->name); ?></p>
                                        <span class="text-[10px] font-bold text-gray-400"><?php echo e($leaveType->code); ?></span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-3 py-3 text-center">
                                <span class="text-[13px] font-bold text-gray-700"><?php echo e($leaveType->default_days_per_year); ?></span>
                            </td>
                            <td class="px-3 py-3 text-center">
                            ...
                            <td class="px-3 py-3 text-center">
                                <?php if($leaveType->is_carry_forward): ?>
                            ...
                            <td class="px-3 py-3 text-center">
                                <span class="text-[10px] font-extrabold uppercase tracking-wider text-gray-600 bg-gray-50 border border-gray-200 px-2.5 py-1 rounded-md"><?php echo e(ucfirst($leaveType->applicable_gender)); ?></span>
                            </td>
                            <td class="px-3 py-3 text-center">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <button @click="openEdit(<?php echo e($leaveType->toJson()); ?>)"
                                    ...
                                    <button @click="confirmDelete(<?php echo e($leaveType->id); ?>, '<?php echo e($leaveType->name); ?>')"
        ...
        <div class="md:hidden divide-y divide-gray-50 border-t border-gray-50 bg-white">
            <?php $__empty_1 = true; $__currentLoopData = $leaveTypes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $leaveType): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="p-4 hover:bg-gray-50/50 transition-colors flex flex-col gap-3"
                     x-show="matchesSearch('<?php echo e(strtolower(addslashes($leaveType->name))); ?>')">
                    
                    
                    <div class="flex justify-between items-start gap-2">
                        <div class="min-w-0">
                            <p class="font-bold text-[14px] text-gray-900 truncate"><?php echo e($leaveType->name); ?></p>
                            <p class="text-[11px] text-gray-500 mt-0.5 font-mono">Code: <?php echo e($leaveType->code); ?></p>
                        </div>
                    ...
                    <div class="flex flex-col gap-2 bg-gray-50/80 px-3 py-2.5 rounded-lg border border-gray-100">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-[14px] font-black text-gray-800"><?php echo e($leaveType->default_days_per_year); ?></span>
                        </div>
                        <div class="flex flex-wrap items-center gap-2 pt-1 border-t border-gray-100/50 mt-1">
                            ...
                            <span class="text-[9px] font-extrabold uppercase tracking-wider text-gray-600 bg-white border border-gray-200 px-1.5 py-0.5 rounded ml-auto">
                                Gender: <?php echo e(ucfirst($leaveType->applicable_gender)); ?>
                    ...
                    <div class="flex items-center justify-end gap-2 pt-1 border-t border-gray-50 mt-1">
                        <button @click="openEdit(<?php echo e($leaveType->toJson()); ?>)" class="w-8 h-8 rounded-lg flex items-center justify-center bg-blue-50 text-blue-500 hover:bg-blue-100 hover:text-blue-700 transition-colors" title="Edit">
                            <i data-lucide="pencil" class="w-4 h-4"></i>
                        </button>
                        <button @click="confirmDelete(<?php echo e($leaveType->id); ?>, '<?php echo e(addslashes($leaveType->name)); ?>')" class="w-8 h-8 rounded-lg flex items-center justify-center bg-red-50 text-red-400 hover:bg-red-100 hover:text-red-600 transition-colors" title="Delete">
        ...
        <?php if($leaveTypes->hasPages()): ?>
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50"><?php echo e($leaveTypes->links()); ?></div>
        <?php endif; ?>
    </div>
...
window.leaveTypePage = function() {
    return {
        ...
        async submitForm() {
            ...
            const url = this.isEditing ? `<?php echo e(url('admin/hrm/leave-types')); ?>/${this.editId}` : `<?php echo e(route('admin.hrm.leave-types.store')); ?>`;
            try {
        ...
        confirmDelete(id, name) {
            BizAlert.confirm('Delete Leave Type', `Delete "${name}"?`, 'Delete').then(async (r) => {
                ...
                try {
                    const resp = await fetch(`<?php echo e(url('admin/hrm/leave-types')); ?>/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
                    const data = await resp.json();
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlink-saas - Slug Based\resources\views/admin/hrm/leave-types/index.blade.php ENDPATH**/ ?>