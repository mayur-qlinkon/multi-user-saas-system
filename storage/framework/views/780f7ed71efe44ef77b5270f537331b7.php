...
<div class="pb-10" x-data="salaryComponentPage()">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
        <div class="stat-card">
            ...
            <p class="text-2xl font-black text-gray-900"><?php echo e($components->total()); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Earnings</p>
            <p class="text-2xl font-black text-green-600"><?php echo e($components->where('type', 'earning')->count()); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Deductions</p>
            <p class="text-2xl font-black text-red-500"><?php echo e($components->where('type', 'deduction')->count()); ?></p>
        </div>
        <div class="stat-card">
            <p class="text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Statutory</p>
            <p class="text-2xl font-black text-gray-900"><?php echo e($components->where('is_statutory', true)->count()); ?></p>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-100 rounded-2xl px-4 py-3 mb-4">
        <div class="flex items-center gap-3 flex-wrap">
            ...
            <?php if(has_permission('salary_components.create')): ?>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="block md:table-row-group">
                    <?php $__empty_1 = true; $__currentLoopData = $components; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $component): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="block md:table-row border-b border-gray-100 mb-4 md:mb-0 p-4 md:p-0" x-show="matchesSearch('<?php echo e(strtolower($component->name)); ?>', '<?php echo e(strtolower($component->code)); ?>')">
                            ...
                            <td class="block md:table-cell px-2 md:px-5 py-1 md:py-3 text-[12px] font-bold text-gray-400">
                                <div class="flex items-center justify-between md:hidden mb-2">
                                    <span>#<?php echo e($components->firstItem() + $loop->index); ?></span>
                                    <div class="flex items-center gap-1.5">
                                        <?php if(has_permission('salary_components.update')): ?>
                                            <button @click="openEdit(<?php echo e($component->toJson()); ?>)" class="w-[28px] h-[28px] rounded-lg flex items-center justify-center bg-blue-50 text-blue-500">
                                                <i data-lucide="pencil" class="w-3.5 h-3.5"></i>
                                            </button>
                                        <?php endif; ?>
                                        <?php if(has_permission('salary_components.delete')): ?>
                                            <button @click="confirmDelete(<?php echo e($component->id); ?>, '<?php echo e($component->name); ?>')" class="w-[28px] h-[28px] rounded-lg flex items-center justify-center bg-red-50 text-red-400">
                                                <i data-lucide="trash-2" class="w-3.5 h-3.5"></i>
                                            </button>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <span class="hidden md:inline"><?php echo e($components->firstItem() + $loop->index); ?></span>
                            </td>
                            ...
                            <td class="block md:table-cell px-2 md:px-5 py-1 md:py-3">
                                <div>
                                    <p class="text-[13px] font-bold text-gray-800">
                                        ...
                                        <span class="text-[11px] font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded ml-1.5"><?php echo e($component->code); ?></span>
                                    </p>
                                    <?php if($component->description): ?>
                                        <p class="text-[11px] text-gray-400 mt-0.5 truncate max-w-[250px]"><?php echo e($component->description); ?></p>
                                    <?php endif; ?>
                                </div>
                            </td>
                            ...
                            <td class="flex justify-between items-center md:table-cell px-2 md:px-3 py-1.5 md:py-3">
                                ...
                                <div class="text-right md:text-left">
                                    <p class="text-[12px] text-gray-600 capitalize"><?php echo e($component->calculation_type); ?></p>
                                    <?php if($component->calculation_type === 'percentage' && $component->percentage_of): ?>
                                        <p class="text-[11px] text-gray-400">of <?php echo e($component->percentage_of); ?></p>
                                    <?php endif; ?>
                                </div>
                            </td>
                            ...
                            <td class="flex justify-between md:table-cell px-2 md:px-3 py-1.5 md:py-3 md:text-right">
                                ...
                                <span class="text-[12px] font-bold text-gray-700"><?php echo e(number_format($component->default_amount ?? 0, 2)); ?></span>
                            </td>
                            ...
                            <td class="hidden md:table-cell px-4 py-3 text-right border-t md:border-t-0 mt-2 md:mt-0 pt-3 md:pt-0">
                                <div class="flex items-center justify-end gap-1.5">
                                    <?php if(has_permission('salary_components.update')): ?>
                                        <button @click="openEdit(<?php echo e($component->toJson()); ?>)" class="w-[30px] h-[30px] rounded-lg flex items-center justify-center bg-blue-50 text-blue-500 hover:bg-blue-100 hover:text-blue-700 transition-colors">
                                            <i data-lucide="pencil" class="w-3.5 h-3.5"></i>
                                        </button>
                                    <?php endif; ?>
                                    <?php if(has_permission('salary_components.delete')): ?>
                                        <button @click="confirmDelete(<?php echo e($component->id); ?>, '<?php echo e($component->name); ?>')" class="w-[30px] h-[30px] rounded-lg flex items-center justify-center bg-red-50 text-red-400 hover:bg-red-100 hover:text-red-600 transition-colors">
                                            <i data-lucide="trash-2" class="w-3.5 h-3.5"></i>
                                        </button>
                                    <?php endif; ?>
                                </div>
    ...
    return {
        ...
        async submitForm() {
            ...
            const url = this.isEditing
                ? `<?php echo e(url('admin/hrm/salary-components')); ?>/${this.editId}`
                : `<?php echo e(route('admin.hrm.salary-components.store')); ?>`;
        ...
        confirmDelete(id, name) {
            BizAlert.confirm('Delete Component', `Are you sure you want to delete "${name}"?`, 'Delete').then(async (result) => {
                ...
                try {
                    const response = await fetch(`<?php echo e(url('admin/hrm/salary-components')); ?>/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                            'Accept': 'application/json',
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlink-saas - Slug Based\resources\views/admin/hrm/salary-components/index.blade.php ENDPATH**/ ?>