<?php $__env->startSection('title', 'Office Locations'); ?>
...
<div class="pb-10" x-data="officeLocations()" x-init="init()">
    ...
    <div class="grid grid-cols-1 gap-4">
        ...
        <div class="loc-card" x-data="locationCard(<?php echo e($store->id); ?>)">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 px-5 py-4 bg-gray-50/60 border-b border-gray-100">
                ...
                <div class="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-[14px] font-black text-gray-900 truncate"><?php echo e($store->name); ?></p>
                ...
                <div class="flex items-center gap-2 flex-wrap justify-start md:justify-end">
                    <?php if($store->office_lat && $store->office_lng): ?>
                    ...
                    <span class="inline-flex items-center gap-1.5 text-[11px] font-bold text-blue-700 bg-blue-50 border border-blue-200 px-2.5 py-1 rounded-full">
                        ...
                        <?php echo e($store->gps_radius_meters ?? 100); ?>m radius
                    </span>
                </div>
            </div>
            ...
            <div class="p-5">
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:items-end">
                    ...
                    <div class="flex flex-wrap items-center gap-2 pt-2 sm:pt-0 w-full">
                        <?php if(has_permission('office_locations.update')): ?>
                        <button @click="saveLocation(<?php echo e($store->id); ?>)" :disabled="saving"
                        ...
                        <?php if(has_permission('office_locations.generate_qr')): ?>
                        <button @click="generateQr(<?php echo e($store->id); ?>, '<?php echo e($store->name); ?>')"
                            :disabled="qrLoading"
        ...
        <?php $lat = $store->office_lat; $lng = $store->office_lng; $radius = $store->gps_radius_meters ?? 100; ?>
        <script>
        (function() {
            const storeId = <?php echo e($store->id); ?>;
            if (!window._locationCards) window._locationCards = {};
            window._locationCards[storeId] = {
                office_lat: <?php echo e($lat ?? 'null'); ?>,
                office_lng: <?php echo e($lng ?? 'null'); ?>,
                gps_radius_meters: <?php echo e($radius); ?>,
            };
        })();
        </script>
...
function locationCard(storeId) {
    ...
    return {
        ...
        async saveLocation(id) {
            ...
            this.errors = {};
            try {
                const resp = await fetch(`<?php echo e(url('admin/hrm/office-locations')); ?>/${id}`, {
                    method: 'PUT',
                    headers: {
        ...
        async generateQr(id, name) {
            this.qrLoading = true;
            try {
                const resp = await fetch(`<?php echo e(url('admin/hrm/office-locations')); ?>/${id}/generate-qr`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Accept': 'application/json',
...
window.officeLocations = function() {
    return {
        ...
        printPoster() {
            ...
            window.open(`<?php echo e(url('admin/hrm/office-locations')); ?>/${this.qrStoreId}/poster`, '_blank');
        },
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\qlinkongraphics\Desktop\MyLab\qlinkonSoftware\resources\views/admin/hrm/office-locations/index.blade.php ENDPATH**/ ?>